...
\cvline{Current Production Experience}{ Python, AWS/EMR, Spark, Airflow, 
  Postgres,  Redshift,  Docker/K8s, Snowflake}
\cvline{Languages}{Python}
\section{Experience}
  \cventry{January 2020 - Present}{ Senior Software Engineer - Data Engineering team}{YouGov}{San
  Francisco, CA}{}{
  \begin{itemize}
  \item Key reponsibilities include: migrating our ETL jobs from
    and in-house platform and K8s to Airflow, 
    % developping data engineering projects from conception to
    % implementation,  monitoring our ETL jobs for
    % failures, administering our Airflow infrastructure, making sure
    % our K8s resource usage is optimized.
  \item Led initiative to migrate most of the ETL jobs of my department from an
    internal platform to Airflow. Today, more than 95\% of the of
    hundred or so of the
    ETL jobs in my department are running on Airflow. Other
    departments have also used our expertise to migrate their ETL jobs
    to Airflow
  \item Led several projects tackling the reliabilty of our existing
    data pipelines. Our improvements have lead to jobs completing
    faster, with fewer interruptions, and better resource usage.
  \item Led several new projects that enabled the
    data science and product team in testing and delivering new
    products to internal and external stakeholders. 
  \item Wrote library that enabled us to export data out of several
    databases (Postgres, MongoDB, DynamoDB) into Snowflake in
    an efficient manner.
  \item Reduced time to run key ETL job from 2 days to less than 12
    hours. Lowered cost of running ETL job by nearly 80\%. The ETL job
    runs every day and enabled YouGov to have access to up-to-date
    numbers during the election period of 2020.    
  \item Provided technical leadership in several backend projects.
  \item Migrated several internal projects to Python3    
  \end{itemize}
  }
